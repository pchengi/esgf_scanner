...
aparser.add_argument('--attachments',type=str,default='none',nargs='?')
...
fls=None
if attachments != 'none':
    fls=attachments.split(',')
fp=codecs.open(body,'r',encoding='utf-8')
text=fp.read()
fp.close()
send_mail(send_from,send_to,subject,text,server=srv,port=prt,files=fls)
